...
class UNIQUE {
    Start_Words_Counter;
    Discription;
    Restart_Word_Counter;
    constructor(startwordcounter, discription, restartwordcounter) {
...
const AskQuestion = async () => {
    const { Start_Words_Counter } = await inquirer.prompt([
        {
...
async function Start_Words_Counting() {
    if (askquestions === "Yes") {
        ...
        await Title(Title1);
        const { Discription } = await inquirer.prompt([
            {
